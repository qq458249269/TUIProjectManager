...
- **滚动查看历史**：滚轮回看滚动缓冲，左键拖拽选择文本；全屏 TUI（opencode /
  jcode 等备用屏应用）也支持滚轮回看历史——这类应用没有仿真器滚动缓冲，且
  Windows ConPTY 会改写宿主写入的 SGR/X10 鼠标序列（滚轮转发给应用不可靠），
  改为在备用屏内容滚动出视口时自攒历史行，滚轮在历史里回看，滚到底回到实时。
...
| 操作 | 功能 |
| --- | --- |
| `滚轮` | 翻看滚动缓冲（历史输出）；全屏 TUI 下回看自攒的备用屏历史，滚到底回到实时 |
| `左键拖拽` + `右键` | 选中 → 右键复制 |
